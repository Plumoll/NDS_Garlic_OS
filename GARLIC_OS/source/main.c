...
void gestionSincronismos()
{
	...
	if (_gd_sincMain & 0xFFFE)		// si hay algun sincronismo pendiente
	{
		...
		for (i = 1; i <= 15; i++)
		{
			...
			{
				_gm_liberarMem(i);
				_gg_escribir("* %d: memoria liberada\n", i, 0, 0);
				_gs_dibujarTabla();
...
int escogerNumero(const unsigned char numeros[], int num_opciones)
{
	...
	{								// mostrar opciones
		if(numeros[j] == 0){
			_gg_escribir("%c: %d (Ocupado)\n", indicadoresTeclas[j], /*numeros[j]*/j+1, 0);
		}else{
			_gg_escribir("%c: %d\n", indicadoresTeclas[j], /*numeros[j]*/j+1, 0);
		}
	}
	return leerTecla(num_opciones)+1;
}
			...
			{*/
				...
				{	if (_gd_pcbs[i].PID == 0)
					{	zocalosDisponibles[j] = i;
						...
						zocalosDisponibles[j] = 0;
						j++;
					}
				}
				_gg_escribir("\n*** seleccionar zocalo :\n", 0, 0, 0);
				zocalo = escogerNumero(zocalosDisponibles, 3/*j*/);
				if(zocalosDisponibles[zocalo-1]==0){
					_gp_matarProc(zocalo);
					_gm_liberarMem(zocalo);
					_gs_dibujarTabla();
					_gg_escribir("* %d: proceso eliminado\n", zocalo, 0, 0);
